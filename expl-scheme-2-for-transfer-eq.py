...
for i in range(1, len(TIME_1)):
    for j in range(1, len(X_list)):
        num_sol[i][j] = -a*(num_sol[i-1][j] - num_sol[i-1][j-1])*tau_1/h + num_sol[i-1][j]
...
for i in range(len(TIME_1)):
    ...
    plt.plot(X_list[20::], an_sol[i][20::])
    plt.scatter(X_list[20::], num_sol[i][20::], c='red', s=10)
    plt.draw()
